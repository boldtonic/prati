...
const QUICK_CHIPS = [
  "What's the strongest setup?",
  'Any sector moving together?',
  'Spot any false positives?',
]
...
function StatusDot() {
  ...
  const dotColor = {
    connecting: 'bg-t3',
    scanning:   'bg-amber animate-pulse',
...
export default function ScanChat() {
  ...
  // Auto-brief on mount
  useEffect(() => {
    sendMessage("What's happening right now? Give me a quick briefing of the scan.", [])
  ...
  async function sendMessage(text: string, history: Message[]) {
    ...
    const isAutoBrief = history.length === 0 && text.startsWith("What's happening right now")
    const now = new Date()
  ...
  return (
    <div className="flex flex-col h-screen bg-bg text-t1 overflow-hidden">
      ...
      <header className="flex items-center gap-3 px-4 h-11 bg-s1 border-b border-bd flex-shrink-0">
        ...
        <span className="font-semibold text-t1 text-sm flex-1">Scan Chat</span>
        <StatusDot />
      </header>
